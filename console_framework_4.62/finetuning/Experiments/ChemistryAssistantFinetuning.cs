...
namespace finetuning.Experiments
{
    internal static class ChemistryAssistantFinetuning
    {
        ...
        private static double ComputeAccuracy(string loraPath, float loraScale, out TimeSpan elapsed, out bool isBest)
        {
            ...
            var model = new LLM(DefaultModelPath);
        ...
        {
            return DownloadContent(new Uri(DatasetURI));
        }
        ...
        {
            List<(string, int)> testingDataset = new List<(string, int)>();
            string[] samples = _trainingDataset.Split(new string[] { "<SFT>" }, StringSplitOptions.RemoveEmptyEntries);
