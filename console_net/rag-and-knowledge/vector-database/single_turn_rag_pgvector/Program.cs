...
namespace single_turn_rag_pgvector
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Initializing store
            // This demo expects a local PostgreSQL instance with the pgvector extension available.
            // Start one with Docker (host port 5433 avoids clashing with a native PostgreSQL on 5432):
            //   docker run --name lmkit-pgvector -e POSTGRES_PASSWORD=postgres -p 5433:5432 -d pgvector/pgvector:pg17
            string connectionString = "Host=localhost;Port=5433;Database=vector_test;Username=postgres;Password=postgres";

            // Create the target database if it does not exist yet (requires a role with the CREATEDB privilege).
            // The 'vector' extension, schema, tables, and indexes are created automatically on first use.
            PgVectorEmbeddingStore.EnsureDatabaseExistsAsync(connectionString).GetAwaiter().GetResult();

            _store = new PgVectorEmbeddingStore(connectionString);
